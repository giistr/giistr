...
const githubOauth = `https://github.com/login/oauth/authorize?client_id=${config.get(
  'clientId'
)}`;
        ...
        <header style={styles.wrapper}>
          <div style={styles.headerContainer}>
            ...
            <div style={styles.first}>
              Giistr give you all the functionalities you need to quickly find
              the right issue.
            </div>
        ...
        <div style={styles.container}>
          <div style={styles.titles}>
            <h1 style={styles.mainTitle}>
              Contribute to build the open-source world.
            </h1>
            <div>
              <h1 style={styles.subTitle}>
                Search, filter and help easily on the issues of the repositories
                you starred
              </h1>
              <BlinkSquare />
            </div>
          ...
          <div style={styles.interactive}>
            <div style={styles.rateLimit}>
              {isTokenAccess
                ? 'Unlimited access using a token'
                : 'Limited access, 5,000 requests per hour'}
            </div>
            {isTokenAccess ? (
              <TokenLogin onClickLogin={this.onTokenLogin} />
            ) : (
              <GithubButton href={githubOauth} />
            )}
            <div style={styles.bottom}>
              <RawButton onClick={this.onToggleAccess}>
                {isTokenAccess ? 'Sign Up with Github' : 'Get unlimited access'}
              </RawButton>
              {isTokenAccess && (
                <RawButton style={styles.howToToken} onClick={this.onGetToken}>
                  How to get a token
                </RawButton>
              )}
            </div>
